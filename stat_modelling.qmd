...
**Goal:** Investigate and quantify risk factors associated with arthritis using data from the CDC Behavioral Risk Factor Surveillance System (BRFSS) in USA.

**Data:** 359,925 US adults surveyed in 2013. Selected Variables: arthritis status, sex, age (65+), physical activity, and BMI.

**Methods:** Chi-square tests, Wilcoxon rank-sum test, relative risk ratios, and multivariate logistic regression.

**Key Findings:**

-   Females had 54% higher odds of arthritis (OR = 1.54, 95% CI: 1.52-1.57)
-   Adults 65+ had 3.5 times the odds of arthritis (OR = 3.56, 95% CI: 3.51-3.62)
-   Physical inactivity increased odds by 45% (OR = 1.45, 95% CI: 1.43-1.47)
-   All associations are significant at p \< 0.001

**Tools:** R, tidyverse, gtsummary, epitools, finalfit, reportROC
...
Selected five variables for analysis: arthritis (outcome), physical activity, BMI, age (65+), and sex.
...
The dataset has zero missing values. The visual below show distribution of the data by target variable.
...
Seeing from the output, the variable bmi, has significant outliers bias the results. In the subsequent section, we are going to remove outliers.
...
In this analysis, we are making the following assumptions about our data: - The data is normally distributed - The expected values in the contingency table for categorical variables are not less than 5 in each cell.
...
1.  Chi-square tests for associations between arthritis and each categorical variable
2.  Wilcoxon rank-sum test for BMI comparison between groups (non-parametric alternative to t-test)
3.  Relative risk ratios for sex, age, and physical activity
4.  Multivariate logistic regression for the collective effect of all predictors

### Hypotheses

**Chi-square tests:**

-   Null: No association between arthritis and sex. Alternative: Association exists.
-   Null: No association between arthritis and physical activity. Alternative: Association exists.
-   Null: No association between arthritis and age category (65+). Alternative: Association exists.
...
-   Chi-square test of indeoendence shows a statistically significant relationship between sex and arthritis (X-squared = 3016, df = 1, p\<0.001), indicating that arthritis prevalence is different in males and females.

-   Risk Ratio: Males have a lower risk of arthritis compared to females (RR = 0.86, 95% CI: 0.86-0.88, p\<0.001). This means that the risk of arthritis in males is about 14% lower than in females.
...
-   Logistic regression: The adjusted OR = 0.94 (95% CI: 0.93-0.94, p \< 0.001) suggests that there is a 6% decrease in the odds of arthritis for every unit of BMI gained. Even as this is significant the result's inverse relationship could suggest possible confounding in our model.
