...
```{r Model Diagnostics, eval=FALSE, include=FALSE}
brfss_roc<-brfss_eliminated |> 
  mutate(arthritis_roc=factor(if_else(arthritis=="Yes",0,1)),
         Sex_roc = factor(if_else(Sex == "Female",0,1)),
         active_roc = factor(if_else(physical_activity == "Yes",0,1)),
         age_roc = factor(if_else(age_65_or_over == "Yes",0,1)))

reportROC::reportROC(gold = brfss_roc$arthritis_roc,predictor.binary = brfss_roc$.,plot = T,positive = 's')

```

```{r eval=FALSE, include=FALSE}
clr<-brfss_eliminated |> 
  mutate(arthritis_clr=if_else(arthritis=="Yes",0,1))

brfss_clr<-clogit(arthritis_clr~Sex + physical_activity, strata(arthritis_clr),data=clr)

summary(brfss_clr)
anova(clr,test="Chisq")
```
